@extends('template.template')
@section('content')

<section id="about" class="wow fadeInUp">
    <div class="contentFilee">
        <div class="container mt-4">
            <div class="row">
                <div class="col-lg-8">
                    <input type="text" name="nama_file" id="nama_file" class="form-control" placeholder="" value="">
                </div>
                <button type="submit" class="btn btn-primary" id="showdata" data-1="search">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-search" width="15" height="15" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M10 10m-7 0a7 7 0 1 0 14 0a7 7 0 1 0 -14 0"></path>
                        <path d="M21 21l-6 -6"></path>
                    </svg>
                </button>
                <!-- <a href="/download/upload" class="btn btn-md btn-primary mb-3 mt-3 float-end">Upload File</a> -->
                <div class="container">
                    <table id="myTable" class="table table-bordered mt-3">
                        <thead>
                            <tr>
                                <th scope="col" data-1="nameFile"></th>
                                <th scope="col" data-1="action"></th>
                            </tr>
                        </thead>
                        <tbody id="loadtable">
                            <!-- @forelse ($files as $file)
                    <tr>
                        <td>{{ $file->original_name }}</td>
                        <td class="text-center">
                            <a href="{{ route('download.unduh', $file) }}" class="btn btn-sm btn-primary">Download</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="2" class="text-muted text-center">Data file belum tersedia</td>
                    </tr>
                    @endforelse -->
                        </tbody>

                    </table>
                </div>
            </div>
        </div>
</section>
@endsection
@push('myscript')
<script>
    // var $ = jQuery;
    $(function() {
        $("#showdata").click(function(e) {
            var nama_file = $("#nama_file").val();
            if (nama_file == "") {
                Swal.fire({
                    title: 'Warning!',
                    text: 'Keyword must be filled',
                    icon: 'warning',
                    confirmButtonText: 'OK'
                }).then((result) => {
                    $("#nama_file").focus();
                })
                return false;
            }
            $.ajax({
                type: 'POST',
                url: '/downloadfile',
                data: {
                    _token: "{{ csrf_token() }}",
                    nama_file: nama_file
                },
                cache: false,
                success: function(respond) {
                    $("#loadtable").html(respond);
                }
            });
        });
    });

    // jQuery(document).ready(function($) {
    //     $(function() {
    //         $("#showdata").click(function(e) {
    //             var nama_file = $("#nama_file").val();
    //             $.ajax({
    //                 type: 'POST',
    //                 url: '/getdata',
    //                 data: {
    //                     _token: "{{ csrf_token() }}",
    //                     nama_file: nama_file
    //                 },
    //                 cache: false,
    //                 success: function(respond) {
    //                     $("#loadtable").html(respond);
    //                 }
    //             });
    //         });
    //     });
    // })
</script>
@endpush